-- Full order lifecycle + support tickets + returns migration
...
begin
  insert into public.order_status_logs (
    order_id, from_status, to_status, changed_by, changed_by_id, reason, metadata
  ) values (
    p_order_id, p_from_status, p_to_status, p_changed_by, p_changed_by_id, p_reason, p_metadata
  )
...
begin
  ...
  insert into public.support_tickets (
    ...
    subject, description, category, source, order_id, order_number
  ) values (
    v_ticket_number, p_customer_name, p_customer_phone, p_customer_email,
    p_subject, coalesce(p_description, p_subject), p_category, p_source,
...
begin
  insert into public.return_requests (
    order_id, order_number, user_id, customer_name, customer_phone,
    customer_email, reason, details, items
  ) values (
    p_order_id, p_order_number, p_user_id, p_customer_name, p_customer_phone,
    p_customer_email, p_reason, p_details, p_items
  )
  returning id into v_return_id;
  update public.orders
  set return_status = 'requested', updated_at = timezone('utc', now())
  where id = p_order_id;
  return v_return_id;
end;
$ret$;
