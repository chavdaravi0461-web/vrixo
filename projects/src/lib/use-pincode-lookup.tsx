...
export function usePincodeLookup(pin: unknown, options?: { debounceMs?: number }) {
  ...
  const [state, setState] = useState<HookState>({ loading: false, error: null, result: null });
  ...
  useEffect(() => {
    ...
    if (!/^[0-9]{6}$/.test(stablePin)) {
      setState({ loading: false, error: stablePin.length === 0 ? null : "Enter a valid 6-digit pincode.", result: null });
      return;
    }
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (!mounted) return;
        if (!res) {
          setState({ loading: false, error: "Pincode not found.", result: null });
        } else {
          setState({ loading: false, error: null, result: res });
        }
      } catch (err) {
        if (!mounted) return;
        setState({ loading: false, error: "Lookup failed.", result: null });
      }
    }, debounceMs);
